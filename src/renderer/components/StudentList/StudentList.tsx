		...
		<ul className="student-list">
			{students.map(student => (
				<li key={student.id} className="student-list__item">
					<div className="student-list__avatar">{student.name.charAt(0).toUpperCase()}</div>
					<div className="student-list__info">
						<span className="student-list__name">{student.name}</span>
						{student.reaction && <ReactionBadge reaction={student.reaction} size="small" />}
					</div>
					<div className="student-list__status">
						<span
							className={`student-list__indicator ${student.connected ? "student-list__indicator--online" : "student-list__indicator--offline"}`}
						/>
					</div>
					{showKickButton && onKick && (
						<button className="student-list__kick" onClick={() => onKick(student.id)} title={PL.kickStudent}>
